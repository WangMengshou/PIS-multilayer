...
class MCMC():
    def __init__(self, para_len, device, delta_t = 0.1):
       self.para_len = para_len
       self.device = device
       self.delta_t = delta_t

    def update_epidemic_parameters(self, features_state_tensor, epi_paras, soc_attention, P_edge_index, I_edge_index, communities):
        ...
        ga, ma, ha = soc_attention[0], soc_attention[1], soc_attention[2]
    ...
    def update(self, features_state_tensor, epi_paras, soc_attention, P_edge_index, I_edge_index, communities, obs = None):
      r, qA, qU, delta, sigma, muH = self.update_epidemic_parameters(features_state_tensor, epi_paras,
                                                                      soc_attention, P_edge_index, I_edge_index, communities)
...
if __name__ == "__main__":
  ...
  mcmc = MCMC(para_len, device)
  time_scale = 1000
  delays = 0
  start = time.time()
  features_times, _, _ = graph_dynamic_delays(time_scale, mcmc, features_state_tensor, epi_paras,   
                                              soc_paras, P_edge_index, I_edge_index, communities, device, delays)
  print(features_times)
